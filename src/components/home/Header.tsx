...
const Header = ({ className, id }: HeaderProps) => {
  ...
  return (
    <header
      id={id}
      className={clsx(
        'flex justify-between items-center sticky top-0 inset-x-0 px-12 py-5 z-10 gap-6 bg-[#FCFAFF]',
        'transition-all duration-300 ease-in-out',
        popUp && 'backdrop-blur-md bg-[#FCFAFF]/80 shadow',
        className,
      )}
    >
      <a href="#home">
      ...
      <Transition appear show={isMenuOpen} as={Fragment}>
        <Dialog as="div" className="relative z-10" onClose={handleCloseMenu}>
          <div className="fixed inset-0 overflow-y-auto">
            ...
            >
              <Dialog.Panel className="flex flex-col items-stretch px-10 py-12 w-full h-full transform overflow-hidden bg-[#FCFAFF] transition-all text-center">
